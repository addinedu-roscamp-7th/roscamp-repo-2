...
class JsonService(Node):
    def __init__(self):
        ...
        self.get_logger().info('Service [/robot/pickup] ready.')
    ...
    def cb(self, request, response):
        ...
        self.get_logger().info(f'task_name: {task_name}, book_info: {book_info}, location:{location}, storage_info: {storage_info}')        
        ...
        response.ok = True
        response.message = json.dumps(response_data, ensure_ascii=False)
        return response
