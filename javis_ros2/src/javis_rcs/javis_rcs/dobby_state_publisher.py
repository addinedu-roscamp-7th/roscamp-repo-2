...
from javis_interfaces.msg import DobbyLocation
import itertools
...
class DobbyStatePublisher(Node):
    def __init__(self, namespace):
        ...
        self.location_pub = self.create_publisher(DobbyLocation, 'status/robot_location', 10)
        ...
        self.loc_timer = self.create_timer(0.5, self.publish_location)
    ...
    def publish_location(self):
        loc_msg = DobbyLocation()
        loc_msg.robot_name = self.get_namespace()
        loc_msg.robot_loaction.x = 1.0  # 예시 x 좌표
        loc_msg.robot_loaction.y = 2.0  # 예시 y 좌표
        loc_msg.robot_loaction.theta = 0.0  # 예시 방향 (라디안)
        self.get_logger().info(f'위치 : x={loc_msg.robot_loaction.x}, y={loc_msg.robot_loaction.y}, theta={loc_msg.robot_loaction.theta}')
...
def main():
    ...
    if len(sys.argv) < 2:
        # 사용법을 출력하고 노드를 종료합니다.
        print("Usage: ros2 run javis_rcs dobby_state_publisher <namespace>")
        return

    namespace = sys.argv[1]
    node = DobbyStatePublisher(namespace=namespace)
    try:
