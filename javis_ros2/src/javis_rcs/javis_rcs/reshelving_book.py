...
from javis_interfaces.action import ReshelvingBook as RB
...
class ReshelvingBook(Node):
   
    def __init__(self, namespace: str):
        super().__init__('reshelving_book', namespace=namespace)

        self._client = ActionClient(self, RB, 'reshelving_book')
    ...
    def run_task(self, book_id: str, **kwargs) -> Future:
        ...
        return_desk_id = int(kwargs.get('return_desk_id'))
        return_desk_location = self._make_pose2d(kwargs.get('return_desk_location', {}))
        return_desk_pose = self._make_pose(kwargs.get('return_desk_pose', {}))

        # Goal 전송 후, goal 응답 완료 콜백 체인 연결
        goal_future = self.send_goal(return_desk_id=return_desk_id, return_desk_location=return_desk_location, return_desk_pose=return_desk_pose)
...
def main(args=None):
    ...
    if len(sys.argv) < 3:
        print("Usage: ros2 run javis_rcs reshelving_book <robot_namespace> <return_desk_id>")
        return

    robot_namespace = sys.argv[1]
    return_desk_id = sys.argv[2]

    # ✅ __init__ 시그니처 수정에 맞게 사용
    node = ReshelvingBook(namespace=f'{robot_namespace}/main')

    try:
        node.get_logger().info(f"Starting reshelving_book task for return_desk_id: {return_desk_id}")
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.get_logger().info("ReshelvingBook 노드가 종료됩니다.")
