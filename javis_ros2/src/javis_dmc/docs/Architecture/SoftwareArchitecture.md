JAVIS 음성 인식 통합 시스템 아키텍처 v3.0 (최종 확정안)
...
중앙 서버: 작업 스케줄링, 데이터 관리, Voice API 기반 대화 처리
...
| 구성 요소                           | 위치                 | 역할 및 책임             |
| ------------------------------- | ------------------ | -------------------------------- |
| **voice_api_service**          | Voice API Server (독립 서버) | - 음성 인식(STT)·자연어 이해·응답 생성(LLM)·음성 합성(TTS)을 통합 처리<br>  - 세션 기반 대화 관리 (로봇별 Conversation Context 유지)<br>  - Wake Word 이후 수신한 오디오 패킷을 텍스트로 변환하고 의도를 분석<br>  - DMC 작업 요청을 위한 명령 생성 및 결과 음성 합성<br>  - 외부 OpenAI API 또는 로컬 모델 호출<br>  - Application Service 데이터 조회 (Phase 2) 지원<br>**타입:** HTTP REST Server (FastAPI/Flask)<br>**통신:** HTTP ↔ voice_recognition_controller, HTTP↔ DMC, TCP ↔ App Service (Phase 2) |
...
| 구성 요소                            | 타입               | 역할 및 책임    |
| -------------------------------- | ---------------- | --------------------------------------------- |
| **dobby_main_controller (DMC)**  | ROS2 Python Node | - 도비 로봇 통합 제어 및 오케스트레이션<br>  - Main/Sub State 관리 (SMACH 기반 State Machine)<br>  - 작업 실행 (Executor 패턴)<br>  - 하위 컨트롤러 조율 (DDC, DAC, DVS)<br>  - 음성 세션 상태 관리 (listening_mode)<br>  - 배터리 자동 관리 및 충전 제어<br>  - RCS 작업 할당 수락 (Action Server)<br>  - Voice API Service와의 작업/대화 연계<br>  - 긴급 제어 처리 (정지, 취소, 복귀)<br>  - 로봇 상태 실시간 발행 (10Hz)<br>  - 지원 작업: Pickup Book, Reshelving Book, Guide Person, Clean Seat, Sorting Shelves<br>  - 길안내 작업 요청(rcs) <br>**통신:** ROS2 ↔ 하위 컨트롤러, RCS, voice_recognition_controller, GUI |
| **voice_recognition_controller** | ROS2 Python Node | - 아날로그 음성 입출력 제어 (마이크 캡처, 스피커 출력)<br>  - 로컬 Wake Word 감지 ("도비야") 및 Listening 세션 트리거<br>  - 오디오 스트림을 패킷화하여 voice_api_service에 전달<br>  - Voice API가 생성한 TTS 오디오를 수신해 스피커로 재생<br>  - DMC listening 모드 상태와 동기화 (ROS2 Service)<br>  - 세션 타임아웃/취소 시 아날로그 스트림 중단<br>**통신:** Serial ↔ d_mic & d_speaker, HTTP ↔ voice_api_service, ROS2 ↔ DMC       |
