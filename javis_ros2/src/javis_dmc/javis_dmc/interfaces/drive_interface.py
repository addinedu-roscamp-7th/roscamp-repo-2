...
from javis_interfaces.action import GuideNavigation, PatrolWaypoints
...
class RosDriveInterface(DriveInterface):
    ...
    def initialize(self) -> bool:
        ...
        self._move_client = ActionClient(
            self.node,
            NavigateToPose,
            self._create_topic_name('drive/navigate_to_pose'),
        ...
        self._wait_for_action(self._move_client, 'navigate_to_pose')
    ...
    def move_to_target(self, pose: Pose, location_name: str = '') -> Future:
        ...
        goal.behavior_tree = ''
        if location_name:
            self.logger.info(f'NAV2 이동 목표: {location_name}')
        else:
            self.logger.info('NAV2 이동 목표: (이름 없음)')
