...
class PlaceBookActionClient(Node):
    def __init__(self):
        super().__init__('place_book_action_client')
        self._client = ActionClient(self, PlaceBook, 'dobby2/arm/place_book')
    def send_goal(self):
        # 서버 준비 대기
        self.get_logger().info(':hourglass_flowing_sand: Waiting for PlaceBook action server...')
        self._client.wait_for_server()
        self.get_logger().info(':white_check_mark: PlaceBook action server connected.')
        # Goal 생성
        goal_msg = PlaceBook.Goal()
        goal_msg.book_id = "6"
        ...
        # Goal 전송
        self.get_logger().info(f":rocket: Sending PlaceBook goal: {goal_msg.book_id}")
