...
class PlaceBookActionServer_DAC2(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info("✅ PlaceBookActionServer_DAC2 initialized.")
    ...
    async def _run_place_sequence(self, goal_handle, feedback, result):
        goal = goal_handle.request
        found_target = self.slot_inventory.get_shelf_by_book(int(goal.book_id))
        ...
        for marker_info in markers_info:
            ...
            await self.robot_move.transfer_book("DOBBY_TO_SHELF", shelf_pose, found_target, 2)
...
def main(args=None):
    ...
    node = PlaceBookActionServer_DAC2()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.get_logger().info("🛑 사용자가 중단함 (KeyboardInterrupt).")
    except Exception as e:
        node.get_logger().error("❌ PlaceBookActionServer_DAC2에서 처리되지 않은 예외 발생:")
        node.get_logger().error(f"{type(e).__name__}: {e}")
        node.get_logger().error(traceback.format_exc())
    finally:
        node.get_logger().info("🔻 PlaceBookActionServer_DAC2 종료 중...")
        node.destroy_node()
        rclpy.shutdown()
