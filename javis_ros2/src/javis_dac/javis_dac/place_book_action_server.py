...
HOME_POSE  = [ 90 , 180  , 250. ,  -180.  ,    0. ,   -45.  ]   # 초기 자세 (Z=250)
LOWER_POSE = [ 90 , 180  , 170. ,  -180.  ,    0. ,   -45.  ]   # 하강 자세 (Z=170)
SPEED = 25
...
class PlaceBookActionServer(Node):
    def __init__(self):
        ...
        self.get_logger().info("✅ PlaceBook Action Server initializing...")
        ...
        try:
            ...
            time.sleep(2.0)
            self.get_logger().info("✅ PlaceBook Action Server Ready (Fixed Z-pose mode)")
    ...
    def execute_callback(self, goal_handle):
        ...
        self.get_logger().info(
            f"📦 PlaceBook goal received → Book ID: {book_id}, "
            f"carrier_slot={goal_handle.request.carrier_slot_id}, "
            f"storage_box={goal_handle.request.storage_box_id}"
        )
        ...
        try:
            ...
            # 3️⃣ 배치 시뮬레이션
            publish("PLACING", "Simulating book placement...")
            time.sleep(1.5)
            current = self.mc.get_coords()
            if current:
                self.get_logger().info(f"📖 배치 위치: {current}")
            ...
            result.storage_box_id = goal_handle.request.storage_box_id
            result.message = "✅ Fixed-pose place sequence completed successfully."
            ...
            result.book_id = book_id
            result.storage_box_id = goal_handle.request.storage_box_id
            result.message = f"❌ PlaceBook failed: {e}"
