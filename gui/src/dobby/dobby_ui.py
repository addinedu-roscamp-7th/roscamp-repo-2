...
# 스크립트 파일이 위치한 디렉토리를 기준으로 GIF 경로를 동적으로 설정
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
print(f"SCRIPT_DIR: '{SCRIPT_DIR}'")
# asset 폴더가 dobby 폴더의 상위 폴더에 있으므로, 상위 폴더 경로를 기준으로 설정
BASE_ASSET_DIR = QDir(SCRIPT_DIR).path() + '/.../asset/image' 
print(f"BASE_ASSET_DIR: '{BASE_ASSET_DIR}'")


# UI파일 연결
form_window = uic.loadUiType("dobby_ui.ui")[0]
...
class WindowClass(QMainWindow, form_window) :
    def __init__(self) :
        ...
        self.movie_loading = QMovie('image/Loading.gif', QByteArray(), self)
        self.movie_charging = QMovie('image/Charging.gif', QByteArray(), self)
        self.movie_walking = QMovie('image/Walking.gif', QByteArray(), self)
    ...
    def imgChange(self, button, checked):
        ...
        current_movie = self.movie_map.get(button_id) # ID를 통해 해당 QMovie 객체 조회
        
        # 현재 실행 중인 무비 설정 및 시작
        if current_movie and current_movie.isValid():
            # setMovie()가 자동으로 이전 무비를 끊으므로 별도의 stop()은 필요하지 않음.
            self.label_img.setMovie(current_movie)
            current_movie.start()
            print(f"상태 변경: ID {button_id} (파일: {os.path.basename(current_movie.fileName())})")
        else:
            self.label_img.setText(f"GIF 로드 실패: ID {button_id} 에 해당하는 파일 경로 또는 유효성 확인 필요")
